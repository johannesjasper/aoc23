...
pub fn puzzle1(input: &str) -> i32 {
    return input.lines().flat_map(parse_line).sum();
}
pub fn puzzle2(input: &str) -> i32 {
    return input.lines().flat_map(parse_line_part2).sum();
}

fn parse_line(input: &str) -> Option<i32> {
    ...
    let number_string = String::from_iter([first_digit, last_digit]);

    number_string.parse::<i32>().ok()
}

fn parse_line_part2(input: &str) -> Option<i32> {
    let result = input
    ...
    return parse_line(result.as_str());
}

pub fn input_day_1() -> String {
    return "nkzjrdqrmpztpqninetwofour1znnkd
